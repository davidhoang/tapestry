...
- **MCP Designer Enrichment** (January 2026): Model Context Protocol integration for AI-assisted profile enrichment. Allows external AI tools (Claude, ChatGPT, etc.) to manage designers through natural language. Features:
  - Token-based authentication with role validation
  - Tools: `enrich_designer`, `enrich_designer_from_url`, `apply_enrichment`, `bulk_enrich_designers`
  - URL enrichment fetches actual page content (personal portfolios, Dribbble, Behance work best; LinkedIn blocks automated access)
  - Enrichment workflow: search/select designer -> enrich -> review suggestions -> apply changes
  - Full documentation in server/mcp/README.md
